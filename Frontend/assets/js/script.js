...
function updateContentForFidelity() {
  ...
  if (currentFidelity === "wireframe") {
    ...
    heroDescription.textContent =
      "Professional waste collection services for your home. We make recycling easy and convenient while protecting our environment.";
...
function selectWasteType(type) {
  ...
  document.querySelectorAll(".selection-card").forEach((card) => {
    if (
      card.id.includes("organic") ||
      card.id.includes("plastic") ||
      card.id.includes("electronic")
    ) {
...
function selectAmount(amount) {
  ...
  document.querySelectorAll(".selection-card").forEach((card) => {
    if (
      card.id.includes("small") ||
      card.id.includes("medium") ||
      card.id.includes("large")
    ) {
      card.classList.remove("selected");
    }
  });
...
function calculatePrice() {
  ...
  const basePrice =
    serviceData.wasteType === "electronic"
      ? 25
      : serviceData.wasteType === "plastic"
      ? 15
      : 12;
  const multiplier =
    serviceData.amount === "large"
      ? 2
      : serviceData.amount === "medium"
      ? 1.5
      : 1;
...
function uploadPhoto() {
  ...
  document.getElementById("photo-count").textContent = `${
    serviceData.photos
  } photo${serviceData.photos > 1 ? "s" : ""} uploaded`;
}
...
function updateCheckoutSummary() {
  document.getElementById("summary-waste-type").textContent =
    serviceData.wasteType
      ? serviceData.wasteType.charAt(0).toUpperCase() +
        serviceData.wasteType.slice(1)
      : "Not selected";

  document.getElementById("summary-amount").textContent = serviceData.amount
    ? serviceData.amount.charAt(0).toUpperCase() + serviceData.amount.slice(1)
    : "Not selected";
...
// Función para simular autenticación de usuario
function login() {
  ...
  localStorage.setItem("userLogged", email); // puedes almacenar más si lo deseas
  showScreen("home");
